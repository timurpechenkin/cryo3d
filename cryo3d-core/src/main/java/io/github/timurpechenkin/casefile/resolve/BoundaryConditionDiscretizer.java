...
public final class BoundaryConditionDiscretizer {
    ...
    public BoundaryConditionField discretize(Grid3D grid,
        ...
        return new BoundaryConditionField(result);
    }

    private int[] discretizeFace(Grid3D grid, Face face, Field<String> field, BoundaryConditionLibrary lib) {
        ...
        return switch (face) {
            case X_MIN -> {
                int x2 = grid.edgesScaled(Axis3D.X)[0] * 2;
                yield discretizeX(grid, field, lib, rules, x2);
            }
            case X_MAX -> {
                int x2 = grid.edgesScaled(Axis3D.X)[nx] * 2;
                yield discretizeX(grid, field, lib, rules, x2);
            }
            case Y_MIN -> {
                int y2 = grid.edgesScaled(Axis3D.Y)[0] * 2;
                yield discretizeY(grid, field, lib, rules, y2);
            }
            case Y_MAX -> {
                int y2 = grid.edgesScaled(Axis3D.Y)[ny] * 2;
                yield discretizeY(grid, field, lib, rules, y2);
            }
            case Z_MIN -> {
                int z2 = grid.edgesScaled(Axis3D.Z)[0] * 2;
                yield discretizeZ(grid, field, lib, rules, z2);
            }
            case Z_MAX -> {
                int z2 = grid.edgesScaled(Axis3D.Z)[nz] * 2;
                yield discretizeZ(grid, field, lib, rules, z2);
            }
        };
    }
    ...
    private int[] discretizeZ(Grid3D grid,
            Field<String> field,
            BoundaryConditionLibrary lib,
        ...
        for (int y = 0; y < ny; y++) {
            ...
            for (int x = 0; x < nx; x++) {
                int x2 = grid.centersScaled2(Axis3D.X)[x];
                int idx = x + nx * y;
    ...
    private int[] discretizeX(Grid3D grid,
            Field<String> field,
            BoundaryConditionLibrary lib,
    ...
    private int[] discretizeY(Grid3D grid,
            Field<String> field,
            BoundaryConditionLibrary lib,
